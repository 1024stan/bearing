...
def optimize_lstm(train_loader=None, test_loader=None,n_iter=4,history_len=None, output_size=None, input_size=None,):
    '''

    :param train_loader: 训练集
    :param test_loader: 测试集
    :param n_iter: 我也不知道啥用，变大可以增多优化次数
    :return: 最优超参数及其得分
    '''
    def lstm_crossval(hidden_size, lstm_layers, learning_rate):
        '''

        :param hidden_size: 待优化参数之 隐藏层
        :param lstm_layers: 待优化参数之 lstm的层数
        :param learning_rate: 待优化参数之 学习率
        :return: 模型在上述参数下的得分，得分计算公式可以修改
        '''
        return run_model(hidden_size=int(hidden_size),
                         lstm_layers=int(lstm_layers),
                         learning_rate=float(learning_rate),
                         history_len=history_len, output_size=output_size, input_size=input_size,
                         train_loader=train_loader, test_loader=test_loader,
                         max_epoch=5,use_bidirectional=False,use_attention=True)

    optimizer = BayesianOptimization(
        f=lstm_crossval,
        pbounds={"hidden_size": (128, 256), "lstm_layers": (2, 8), "learning_rate":(0.0001, 0.01)}, ## 设置超参数及其范围，这个后续可以改
        random_state=1234,
        verbose=2
    )
...
if __name__ == "__main__":
    ...
    # 优化lstm模型
    best_hyperparameters = optimize_lstm(train_loader=train_loader, test_loader=valid_loader,n_iter=4,
